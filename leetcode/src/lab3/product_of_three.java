package lab3;

public class product_of_three {
    public static void main(String argv[]) {
    	int nums[] = {722,634,-504,-379,163,-613,-842,-578,750,951,-158,30,-238,-392,
    			-487,-797,-157,-374,999,-5,-521,-879,-858,382,626,803,-347,903,-205,57,
    			-342,186,-736,17,83,726,-960,343,-984,937,-758,-122,577,-595,-544,-559,
    			903,-183,192,825,368,-674,57,-959,884,29,-681,-339,582,969,-95,-455,-275,
    			205,-548,79,258,35,233,203,20,-936,878,-868,-458,-882,867,-664,-892,-687,
    			322,844,-745,447,-909,-586,69,-88,88,445,-553,-666,130,-640,-918,-7,-420,
    			-368,250,-786}; 
        int max[] = {-1001, -1001, -1001};
        int max_i[] = {-1, -1, -1};
        int min[] = {1001, 1001, 1001};
        int min_i[] = {-1, -1, -1};
        for (int j = 0; j < 3; j++) {   
            for (int i = 0; i < nums.length; i++) {
                if (nums[i] > max[j] && i != max_i[0] && i != max_i[1]) {
                        max[j] = nums[i];
                    	System.out.println(max[j]);
                        max_i[j] = i;
                }
                if (nums[i] < min[j] && i != min_i[0] && i != min_i[1]) {
                        min[j] = nums[i];
                    	System.out.println(min[j]);
                        min_i[j] = i;
                }
            }  
        }

    	System.out.println(min[0] + " " + min[1] + " " + min[2] + " " + max[0] + " " + max [1] + " " + max[2]);
        if (min[0] * min [1] > max[0] * max [1])
        	System.out.println( min[0] * min [1] * max[0]);
        else
        	System.out.println( max[0] * max [1] * max[2]);
    }
}
